...
public class GT_MetaTileEntity_PortableTank extends GT_MetaTileEntity_BasicTank {
	...
	public String[] getDescription() {
		...
		if (this.mFluid == null) {
			...
			desc[mDescriptionArray.length] = "A portable tank";
		}
		else {
			desc = new String[mDescriptionArray.length + 1];
			System.arraycopy(mDescriptionArray, 0, desc, 0, mDescriptionArray.length);
			desc[mDescriptionArray.length] = "A portable tank";
			desc[mDescriptionArray.length] = "Fluid: "+this.mFluid.getLocalizedName()+" "+Integer.toString(this.mFluid.amount)+"L";
	...
	public MetaTileEntity newMetaEntity(final IGregTechTileEntity aTileEntity) {
		return new GT_MetaTileEntity_PortableTank(this.mName, this.mTier, null, this.mTextures);
	}
